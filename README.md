...
- 可选在线探测；默认只做本地检查。
- 支持 xAI、Codex / ChatGPT 与 Gemini 的可选 Token 刷新或重新授权。
- 支持自动授权巡检异常账号；xAI 与 Codex / ChatGPT 会优先复用已有刷新凭据，
  必要时隔离启动 Chromium 完成 OAuth 流程。
- 支持批量启用或禁用认证文件。
...
巡检首页会固定展示所有摘要卡片，即使当前计数为 `0` 也不会隐藏，便于确认某类
结果确实已被加载。当前摘要包含：
...
| `总数` | 当前扫描到的认证文件总数 |
| `正常` | 本地检查或在线探测可用 |
| `已禁用` | 文件内 `disabled` 为 `true` |
| `Token 过期` | access token 已过期 |
| `待刷新` | 文件具备刷新条件，建议重新授权或刷新 |
| `无效` | 凭据格式异常、缺少必要字段，或上游判定不可用 |
| `无额度` | 在线探测返回额度不足或余额不可用 |
| `403` | 在线探测被上游拒绝访问，通常表示账号、权限、地区、风控或模型访问受限 |
| `429` | 在线探测触发上游限速 |
| `错误` | 本地读取、解析或请求过程中发生未归类错误 |
| `其他` | 不属于以上分类的状态 |

`无额度`、`403` 和 `429` 只有在开启在线探测后才有可靠计数；纯本地扫描不会向
上游发请求，因此这些卡片可能保持 `0`。鼠标悬停在摘要卡片上可查看对应解释。
...
感谢 CLIProxyAPI / CLIProxyAPIPlus 社区长期沉淀的插件体系、认证文件实践和问题反馈；
本项目的自动授权链路也参考了[LINUX DO 社区](https://linux.do/)里围绕 `grok_bytao`、SSO 凭据导出和浏览器
OAuth 自动化的探索。很多状态分类、批量巡检和异常展示细节，来自真实账号池运维
里的反复排查与反馈。感谢所有愿意测试、提 issue、补充场景和分享经验的朋友。
